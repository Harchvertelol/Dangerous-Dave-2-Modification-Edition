...
void GameInfo::correctionScreen(CreaturePlayer* player)
{
    ...
    if(player->s_CoordX - player->s_ScreenCoordX < atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenXleft").c_str()) * 16) player->s_ScreenCoordX = player->s_CoordX - atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenXleft").c_str()) * 16;
    else if( (player->s_ScreenCoordX + s_GameClass->s_DisplayStruct->s_GameResolutionX) - player->s_CoordX < (atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenXright").c_str()) - 1) * 16) player->s_ScreenCoordX = player->s_CoordX + (atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenXright").c_str()) - 1) * 16 - s_GameClass->s_DisplayStruct->s_GameResolutionX;

    if(player->s_CoordY - player->s_ScreenCoordY < atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenYup").c_str()) * 16) player->s_ScreenCoordY = player->s_CoordY - atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenYup").c_str()) * 16;
    else if( (player->s_ScreenCoordY + s_GameClass->s_DisplayStruct->s_GameResolutionY) - player->s_CoordY < (2 + atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenYdown").c_str())) * 16) player->s_ScreenCoordY = player->s_CoordY + (2 + atoi(s_GameClass->s_IniFile->getValue("gamesettings", "limitshiftscreenYdown").c_str())) * 16 - s_GameClass->s_DisplayStruct->s_GameResolutionY;
